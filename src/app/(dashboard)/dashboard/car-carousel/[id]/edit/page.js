...
export default function EditCarCarousel() {
  ...
  useEffect(() => {
    async function fetchCarousel() {
      try {
        setLoading(true)
        const response = await axios.get(`/api/supabasPrisma/car-carousel/${id}`)
        ...
        setError("Failed to load carousel data. Please try again.")
      } finally {
        setLoading(false)
      }
    }
  ...
  async function handleSubmit(e) {
    ...
    if (!formData.title.trim()) {
      setError("Carousel title is required")
      return
    }

    if (!formData.description.trim()) {
      setError("Carousel description is required")
      return
    }
    ...
    try {
      ...
      const response = await axios.put(`/api/supabasPrisma/car-carousel/${id}`, formData)

      // Handle success response
      setUploadStatus("Carousel updated successfully!")
      setTimeout(() => {
      ...
      if (error.response) {
        setError(error.response.data.error || "Failed to update carousel")
      } else {
    ...
    return (
      <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center">
        <div className="bg-card p-6 rounded-[5px] shadow-lg text-center max-w-md w-full border border-brand-primary/20">
          ...
          <h3 className="font-medium text-lg mb-2">Updating Carousel</h3>
    ...
    <div className="max-w-3xl mx-auto px-4 py-4 relative">
      <div className="mb-4">
        <div className="flex items-center text-xs text-muted-foreground mb-2">
          ...
          <ChevronRight className="h-3 w-3 mx-1" />
          <span className="text-foreground">Edit Carousel</span>
        </div>

        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <div>
            <h1 className="text-xl font-bold tracking-tight text-brand-primary">Edit Car Carousel</h1>
            <p className="text-xs text-muted-foreground">Update the details for this carousel</p>
          </div>
          <Button
            variant="outline"
            size="sm"
      ...
      <Card className="shadow-md border-brand-primary/10 overflow-hidden rounded-[5px]">
        <form onSubmit={handleSubmit} className="rounded-[5px]">
          <CardHeader className="bg-brand-light/30 border-b border-brand-primary/10 py-3 px-4">
            ...
            <CardDescription className="text-xs">Update the details for this carousel</CardDescription>
          </CardHeader>
          ...
          <CardContent className="space-y-4 pt-4 px-4">
            ...
            <div className="space-y-1">
              ...
              <Textarea
                ...
                rows={4}
                className="text-sm resize-none focus-visible:ring-brand-primary rounded-[5px]"
                required
              />
              <p className="text-xs text-muted-foreground">Provide details about this carousel</p>
            </div>
          ...
          <CardFooter className="flex justify-end gap-2 py-3 px-4 border-t bg-muted/20">
            ...
            <Button
              type="submit"
              disabled={submitting || !formData.imageUrl || !formData.title.trim() || !formData.description.trim()}
              className="h-8 text-xs bg-brand-primary hover:bg-brand-primary/90 rounded-[5px]"
            >
              {submitting ? "Updating..." : "Update Carousel"}
            </Button>
          </CardFooter>
        </form>
      </Card>
        ...
        <DialogContent className="sm:max-w-[85vw] max-h-[85vh] overflow-hidden p-0 rounded-[5px]">
          <DialogHeader className="px-4 pt-4 pb-3 border-b">
            ...
            <DialogDescription className="text-xs">
              Choose an image from your gallery to use for this carousel
            </DialogDescription>
          </DialogHeader>
