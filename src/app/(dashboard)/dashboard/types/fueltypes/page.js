...
export default function FuelTypePage() {
  ...
  const handleAdd = async () => {
    ...
    if (!formData.title.trim()) {
      setFormError("English fuel type name is required")
    ...
    try {
      await axios.post("/api/fueltype", formData)
      ...
      setFormError(error.response?.data?.error || "Failed to create fuel type. It may already exist.")
    } finally {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      setFormError(error.response?.data?.error || "Failed to update fuel type.")
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const handleDeleteDirect = async (item) => {
    ...
    setIsSubmitting(true)
    setDeleteMessage("Deleting fuel type")

    try {
      await axios.delete("/api/fueltype", {
        data: { id: item.id },
      })
  ...
  const handleDeleteAll = async () => {
    ...
    setIsPageDisabled(true) // Disable the entire page immediately
    setIsSubmitting(true)
    setDeleteMessage("Deleting all fuel types")

    try {
      // Create an array of promises for each delete operation
      const deletePromises = items.map((item) =>
        axios.delete("/api/fueltype", {
          data: { id: item.id },
        }),
      )
  ...
  const handleDeleteSelected = async () => {
    ...
    setIsPageDisabled(true) // Disable the entire page immediately
    setIsSubmitting(true)
    setDeleteMessage(`Deleting ${selectedItems.length} selected fuel type${selectedItems.length > 1 ? "s" : ""}`)
    ...
    <div className="max-w-5xl mx-auto px-4 py-4 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold mb-1 text-brand-primary">
            {language === "en" ? "Fuel Types" : "أنواع الوقود"}
          </h1>
          <p className="text-gray-500 text-sm">{language === "en" ? "Manage fuel types" : "إدارة أنواع الوقود"}</p>
        </div>
            ...
            <DialogContent className="rounded-[5px] max-w-md">
              <DialogHeader className="text-center">
                <DialogTitle className="text-xl font-bold text-brand-primary">
                  {language === "en" ? "Add New Fuel Type" : "إضافة نوع وقود جديد"}
                </DialogTitle>
                <DialogDescription className="text-gray-500">
                  {language === "en"
                    ? "Fill in the details below to create a new fuel type."
                    : "املأ التفاصيل أدناه لإنشاء نوع وقود جديد."}
                </DialogDescription>
              </DialogHeader>
            ...
            <div className="flex justify-center items-center h-40">
              <div className="text-center text-gray-500">
                <p className="mb-2 text-lg font-medium">
                  {searchQuery ? "No matching fuel types found" : "No fuel types found"}
                </p>
                <p className="text-sm">
                  {searchQuery ? "Try adjusting your search query" : "Get started by adding your first fuel type"}
                </p>
                {!searchQuery && (
                  <Button
                    onClick={() => {
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="rounded-[5px] max-w-md">
          <DialogHeader className="text-center">
            <DialogTitle className="text-xl font-bold text-brand-primary">
              {language === "en" ? "Edit Fuel Type" : "تعديل نوع الوقود"}
            </DialogTitle>
            <DialogDescription className="text-gray-500">
