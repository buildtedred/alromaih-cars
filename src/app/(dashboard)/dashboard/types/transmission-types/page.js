...
export default function TransmissionTypesPage() {
  ...
  const handleAdd = async () => {
    ...
    if (!formData.title.trim()) {
      setFormError("English transmission type name is required")
    ...
    try {
      await axios.post("/api/transmissiontype", formData)
      ...
      setFormError(error.response?.data?.error || "Failed to create transmission type. It may already exist.")
    } finally {
  ...
  const handleEdit = async () => {
    ...
    setIsSubmitting(true)
    try {
      await axios.put("/api/transmissiontype", {
        id: currentItem.id,
      ...
      setFormError(error.response?.data?.error || "Failed to update transmission type.")
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const handleDeleteDirect = async (item) => {
    ...
    setIsSubmitting(true)
    setDeleteMessage("Deleting transmission type")

    try {
      await axios.delete("/api/transmissiontype", {
        data: { id: item.id },
      })
  ...
  const handleDeleteAll = async () => {
    ...
    setIsPageDisabled(true) // Disable the entire page immediately
    setIsSubmitting(true)
    setDeleteMessage("Deleting all transmission types")

    try {
      // Create an array of promises for each delete operation
      const deletePromises = items.map((item) =>
        axios.delete("/api/transmissiontype", {
          data: { id: item.id },
        }),
      )
  ...
  const handleDeleteSelected = async () => {
    ...
    setIsSubmitting(true)
    setDeleteMessage(
      `Deleting ${selectedItems.length} selected transmission type${selectedItems.length > 1 ? "s" : ""}`,
    )
    ...
    <div className="max-w-5xl mx-auto px-4 py-4 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold mb-1 text-brand-primary">
            {language === "en" ? "Transmission Types" : "أنواع ناقل الحركة"}
          </h1>
          <p className="text-gray-500 text-sm">
            {language === "en" ? "Manage transmission types" : "إدارة أنواع ناقل الحركة"}
          </p>
        </div>
            ...
            <DialogContent className="rounded-[5px] max-w-md">
              <DialogHeader className="text-center">
                <DialogTitle className="text-xl font-bold text-brand-primary">
                  {language === "en" ? "Add New Transmission Type" : "إضافة نوع ناقل حركة جديد"}
                </DialogTitle>
                <DialogDescription className="text-gray-500">
                  {language === "en"
                    ? "Fill in the details below to create a new transmission type."
                    : "املأ التفاصيل أدناه لإنشاء نوع ناقل حركة جديد."}
                </DialogDescription>
              </DialogHeader>
            ...
            <div className="flex justify-center items-center h-40">
              <div className="text-center text-gray-500">
                <p className="mb-2 text-lg font-medium">
                  {searchQuery ? "No matching transmission types found" : "No transmission types found"}
                </p>
                <p className="text-sm">
                  {searchQuery
                    ? "Try adjusting your search query"
                    : "Get started by adding your first transmission type"}
                </p>
                {!searchQuery && (
                  <Button
                    onClick={() => {
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="rounded-[5px] max-w-md">
          <DialogHeader className="text-center">
            <DialogTitle className="text-xl font-bold text-brand-primary">
              {language === "en" ? "Edit Transmission Type" : "تعديل نوع ناقل الحركة"}
            </DialogTitle>
            <DialogDescription className="text-gray-500">
