...
export default function ConditionTypesPage() {
  ...
  const handleAdd = async () => {
    ...
    if (!formData.title.trim()) {
      setFormError("English condition type name is required")
    ...
    try {
      await axios.post("/api/conditiontype", formData)
      ...
      setFormError(error.response?.data?.error || "Failed to create condition type. It may already exist.")
    } finally {
  ...
  const handleEdit = async () => {
    ...
    setIsSubmitting(true)
    try {
      await axios.put("/api/conditiontype", {
        id: currentItem.id,
      ...
      setFormError(error.response?.data?.error || "Failed to update condition type.")
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const handleDeleteDirect = async (item) => {
    ...
    setIsSubmitting(true)
    setDeleteMessage("Deleting condition type")

    try {
      await axios.delete("/api/conditiontype", {
        data: { id: item.id },
      })
  ...
  const handleDeleteAll = async () => {
    ...
    setIsPageDisabled(true) // Disable the entire page immediately
    setIsSubmitting(true)
    setDeleteMessage("Deleting all condition types")

    try {
      // Create an array of promises for each delete operation
      const deletePromises = items.map((item) =>
        axios.delete("/api/conditiontype", {
          data: { id: item.id },
        }),
      )
  ...
  const handleDeleteSelected = async () => {
    ...
    setIsPageDisabled(true) // Disable the entire page immediately
    setIsSubmitting(true)
    setDeleteMessage(`Deleting ${selectedItems.length} selected condition type${selectedItems.length > 1 ? "s" : ""}`)
    ...
    <div className="max-w-5xl mx-auto px-4 py-4 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold mb-1 text-brand-primary">
            {language === "en" ? "Condition Types" : "أنواع الحالة"}
          </h1>
          <p className="text-gray-500 text-sm">{language === "en" ? "Manage condition types" : "إدارة أنواع الحالة"}</p>
        </div>
            ...
            <DialogContent className="rounded-[5px] max-w-md">
              <DialogHeader className="text-center">
                <DialogTitle className="text-xl font-bold text-brand-primary">
                  {language === "en" ? "Add New Condition Type" : "إضافة نوع حالة جديد"}
                </DialogTitle>
                <DialogDescription className="text-gray-500">
                  {language === "en"
                    ? "Fill in the details below to create a new condition type."
                    : "املأ التفاصيل أدناه لإنشاء نوع حالة جديد."}
                </DialogDescription>
              </DialogHeader>
          ...
          {loading ? (
            ...
            <div className="flex justify-center items-center h-40">
              <div className="text-center text-gray-500">
                ...
                <p className="text-sm">
                  {searchQuery ? "Try adjusting your search query" : "Get started by adding your first condition type"}
                </p>
                {!searchQuery && (
                  <Button
                    onClick={() => {
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="rounded-[5px] max-w-md">
          <DialogHeader className="text-center">
            <DialogTitle className="text-xl font-bold text-brand-primary">
              {language === "en" ? "Edit Condition Type" : "تعديل نوع الحالة"}
            </DialogTitle>
            <DialogDescription className="text-gray-500">
