...
export default function SafetyRatingPage() {
  ...
  const handleAdd = async () => {
    ...
    if (!formData.title.trim()) {
      setFormError("English safety rating name is required")
    ...
    try {
      await axios.post("/api/safetyrating", formData)
      ...
      setFormError(error.response?.data?.error || "Failed to create safety rating. It may already exist.")
    } finally {
  ...
  const handleEdit = async () => {
    ...
    setIsSubmitting(true)
    try {
      await axios.put("/api/safetyrating", {
        id: currentItem.id,
      ...
      setFormError(error.response?.data?.error || "Failed to update safety rating.")
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const handleDeleteDirect = async (item) => {
    ...
    setIsSubmitting(true)
    setDeleteMessage("Deleting safety rating")

    try {
      await axios.delete("/api/safetyrating", {
        data: { id: item.id },
      })
  ...
  const handleDeleteAll = async () => {
    ...
    setIsPageDisabled(true) // Disable the entire page immediately
    setIsSubmitting(true)
    setDeleteMessage("Deleting all safety ratings")

    try {
      // Create an array of promises for each delete operation
      const deletePromises = items.map((item) =>
        axios.delete("/api/safetyrating", {
          data: { id: item.id },
        }),
      )
  ...
  const handleDeleteSelected = async () => {
    ...
    setIsPageDisabled(true) // Disable the entire page immediately
    setIsSubmitting(true)
    setDeleteMessage(`Deleting ${selectedItems.length} selected safety rating${selectedItems.length > 1 ? "s" : ""}`)
    ...
    <div className="max-w-5xl mx-auto px-4 py-4 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold mb-1 text-brand-primary">
            {language === "en" ? "Safety Ratings" : "تصنيفات السلامة"}
          </h1>
          <p className="text-gray-500 text-sm">
            {language === "en" ? "Manage safety ratings" : "إدارة تصنيفات السلامة"}
          </p>
        </div>
            ...
            <DialogContent className="rounded-[5px] max-w-md">
              <DialogHeader className="text-center">
                <DialogTitle className="text-xl font-bold text-brand-primary">
                  {language === "en" ? "Add New Safety Rating" : "إضافة تصنيف سلامة جديد"}
                </DialogTitle>
                <DialogDescription className="text-gray-500">
                  {language === "en"
                    ? "Fill in the details below to create a new safety rating."
                    : "املأ التفاصيل أدناه لإنشاء تصنيف سلامة جديد."}
                </DialogDescription>
              </DialogHeader>
            ...
            <div className="flex justify-center items-center h-40">
              <div className="text-center text-gray-500">
                <p className="mb-2 text-lg font-medium">
                  {searchQuery ? "No matching safety ratings found" : "No safety ratings found"}
                </p>
                <p className="text-sm">
                  {searchQuery ? "Try adjusting your search query" : "Get started by adding your first safety rating"}
                </p>
                {!searchQuery && (
                  <Button
                    onClick={() => {
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="rounded-[5px] max-w-md">
          <DialogHeader className="text-center">
            <DialogTitle className="text-xl font-bold text-brand-primary">
              {language === "en" ? "Edit Safety Rating" : "تعديل تصنيف السلامة"}
            </DialogTitle>
            <DialogDescription className="text-gray-500">
