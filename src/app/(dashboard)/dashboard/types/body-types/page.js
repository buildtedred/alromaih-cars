...
export default function BodyTypePage() {
  ...
  const handleAdd = async () => {
    ...
    if (!formData.title.trim()) {
      setFormError("English body type name is required")
    ...
    try {
      await axios.post("/api/bodytype", formData)
      ...
      setFormError(error.response?.data?.error || "Failed to create body type. It may already exist.")
    } finally {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      setFormError(error.response?.data?.error || "Failed to update body type.")
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const handleDeleteDirect = async (item) => {
    ...
    setIsSubmitting(true)
    setDeleteMessage("Deleting body type")

    try {
      await axios.delete("/api/bodytype", {
        data: { id: item.id },
      })
  ...
  const handleDeleteAll = async () => {
    ...
    setIsPageDisabled(true) // Disable the entire page immediately
    setIsSubmitting(true)
    setDeleteMessage("Deleting all body types")

    try {
      // Create an array of promises for each delete operation
      const deletePromises = items.map((item) =>
        axios.delete("/api/bodytype", {
          data: { id: item.id },
        }),
      )
  ...
  const handleDeleteSelected = async () => {
    ...
    setIsPageDisabled(true) // Disable the entire page immediately
    setIsSubmitting(true)
    setDeleteMessage(`Deleting ${selectedItems.length} selected body type${selectedItems.length > 1 ? "s" : ""}`)
    ...
    <div className="max-w-5xl mx-auto px-4 py-4 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold mb-1 text-brand-primary">
            {language === "en" ? "Body Types" : "أنواع الهيكل"}
          </h1>
          <p className="text-gray-500 text-sm">{language === "en" ? "Manage body types" : "إدارة أنواع الهيكل"}</p>
        </div>
            ...
            <DialogContent className="rounded-[5px] max-w-md">
              <DialogHeader className="text-center">
                <DialogTitle className="text-xl font-bold text-brand-primary">
                  {language === "en" ? "Add New Body Type" : "إضافة نوع هيكل جديد"}
                </DialogTitle>
                <DialogDescription className="text-gray-500">
                  {language === "en"
                    ? "Fill in the details below to create a new body type."
                    : "املأ التفاصيل أدناه لإنشاء نوع هيكل جديد."}
                </DialogDescription>
              </DialogHeader>
            ...
            <div className="flex justify-center items-center h-40">
              <div className="text-center text-gray-500">
                <p className="mb-2 text-lg font-medium">
                  {searchQuery ? "No matching body types found" : "No body types found"}
                </p>
                <p className="text-sm">
                  {searchQuery ? "Try adjusting your search query" : "Get started by adding your first body type"}
                </p>
                {!searchQuery && (
                  <Button
                    onClick={() => {
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="rounded-[5px] max-w-md">
          <DialogHeader className="text-center">
            <DialogTitle className="text-xl font-bold text-brand-primary">
              {language === "en" ? "Edit Body Type" : "تعديل نوع الهيكل"}
            </DialogTitle>
            <DialogDescription className="text-gray-500">
