...
export default function CarFilterSidebar({ onFilterChange, carModels, filters, language = "en" }) {
  ...
  return (
    <div
      className={`bg-white rounded-lg shadow overflow-hidden flex flex-col h-[calc(100vh-2rem)] ${isRTL ? "rtl" : "ltr"}`}
      style={{ scrollbarWidth: "thin", scrollbarColor: "#71308A transparent" }}
    >
        ...
        <div className="p-4 space-y-6">
          ...
          <Collapsible open={openSections.priceRange} onOpenChange={() => toggleSection("priceRange")}>
            <CollapsibleTrigger className="flex justify-between items-center w-full">
              ...
              <ChevronDown className={`w-4 h-4 transition-transform ${openSections.priceRange ? "rotate-180" : ""}`} />
            </CollapsibleTrigger>
                    ...
                    <button onClick={() => handleYearChange(year)} className="flex items-center focus:outline-none">
                      <div
                        className={`w-4 h-4 rounded-full border-2 border-[#71308A] flex items-center justify-center ${
                          localFilters.year === year ? "bg-[#71308A]" : "bg-white"
                        }`}
                      >
                        {localFilters.year === year && <div className="w-2 h-2 rounded-full bg-white" />}
                      </div>
